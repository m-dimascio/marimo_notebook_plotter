...
def _(mo):
    mo.md(
        """
        # Interactive 3D MOSFET Explorer

        This notebook visualizes MOSFET device physics with:
        - **3D volumetric electron concentration** (like COMSOL)
        - **Animated sweeps** through all operating modes
        - **Play/pause controls** for video-like playback
...
@app.cell
def _(mo):
    mo.md("## 3D Electron Concentration Animation")
    return
...
def _(
    ...
    if sweep_mode.value == "vgs":
        fig_3d = create_vgs_sweep_animation(
            params=device,
            ...
            mesh_resolution=(40, 15, 25),
        )
    else:
        fig_3d = create_vds_sweep_animation(
            params=device,
